...
- 무엇을 측정한 데이터인가?
  - 측정 환경: 일상적인 환경(In-the-wild)에서 스마트폰을 사용하는 불특정 다수의 사용자들로부터 수집된 데이터
  - 측정 내용: 스마트폰에 내장된 센서(가속도계, 자이로, 중력 등)을 통해 사용자의 움직임을 시계열 데이터로 기록
  - 특징: 사용자가 폰을 쥐는 방향, 주머니 위치, 걷는 습관 등 모두 제각각인 '실제 상황' 데이터
- 맞춰야 할 정답(Label)은?
...
| **분석 난이도** | 낮음 (교과서적 데이터) | 상대적으로 높음 (전처리의 역할이 큼) |
<br>
<br>
